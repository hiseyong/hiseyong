<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=200&section=header&text=Imagination+Becomes+Reality%20&fontSize=60" />

<div align="center">

  ### Hi, I'm **Seyong Ahn** 👋
  ECE @ [SUNY Korea](https://www.sunykorea.ac.kr/) (Stony Brook University) · First-year undergraduate

  [![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=22&pause=1000&color=58A6FF&center=true&vCenter=true&width=560&lines=Embedded+Systems+%C2%B7+Communications+%C2%B7+Physical+AI;Building+robots%2C+drones%2C+and+autonomous+systems)](https://git.io/typing-svg)

  [![GitHub](https://img.shields.io/badge/GitHub-hiseyong-181717?logo=github&logoColor=white)](https://github.com/hiseyong)
  [![Email](https://img.shields.io/badge/Email-hiseyong1008%40gmail.com-D14836?logo=gmail&logoColor=white)](mailto:hiseyong1008@gmail.com)
  ![Profile views](https://komarev.com/ghpvc/?username=hiseyong&color=blueviolet&style=flat)

</div>

## About me

I'm an Electrical and Computer Engineering student at **SUNY Korea (Stony Brook University)**, interested in **Embedded Systems**, **Communications**, and **Physical AI**.

I work across robotics, autonomous systems, and signal processing — from sim-to-real RL for quadrupeds and aircraft, to physics-constrained deep learning for modulation classification.

## My tech stack

<p align="left">
  <a>
    <img alt="Python" src="https://img.shields.io/badge/Python-%2314354C.svg?logo=python&logoColor=white">
  </a>
  &emsp;
  <a>
    <img alt="C" src="https://img.shields.io/badge/C-%2300599C.svg?logo=c&logoColor=white">
  </a>
  &emsp;
  <a>
    <img alt="JavaScript" src="https://img.shields.io/badge/JavaScript-%23F7DF1E.svg?logo=javascript&logoColor=black">
  </a>
  &emsp;
  <a>
    <img alt="React" src="https://img.shields.io/badge/React-%2320232a.svg?logo=react&logoColor=%2361DAFB">
  </a>
  &emsp;
  <a>
    <img alt="AWS" src="https://img.shields.io/badge/-AWS-232F3E?logo=amazon-aws&logoColor=white">
  </a>
  &emsp;
  <a>
    <img alt="Linux" src="https://img.shields.io/badge/Linux-%23FCC624.svg?logo=linux&logoColor=black">
  </a>
  &emsp;
  <a>
    <img alt="NGINX" src="https://img.shields.io/badge/-NGINX-yellow?logo=nginx&logoColor=white"/>
  </a>
  &emsp;
  <a>
    <img alt="KiCad" src="https://img.shields.io/badge/KiCad-314CB0?logo=kicad&logoColor=white"/>
  </a>
  &emsp;
  <a>
    <img alt="Git" src="https://img.shields.io/badge/-git-red?logo=git&logoColor=white"/>
  </a>
  &emsp;
  <a>
    <img alt="GitHub" src="https://img.shields.io/badge/-GitHub-black?logo=github&logoColor=white">
  </a>
</p>

## Featured projects

| Project | Focus |
| :--- | :--- |
| Linear Actuator-Based Autonomous Steering System | Embedded control · autonomy |
| Obstacle-Avoidance Autonomous Flight Drone | UAV · perception |
| Sim-to-Real Quadruped Robot Locomotion | RL · robotics |
| RL-Based Fixed-Wing Aircraft Takeoff System | Sim-to-real · aerospace |
| Solid Rocket Development and Launch | Hardware · propulsion |
| WiFi-Based Human Presence/Detection System | Sensing · communications |

## Publications

- **Physics-Constrained ResNet-based Automatic Modulation Classification for Low-SNR Robustness** — *KCC 2026* (accepted, sole author)
- **Reinforcement Learning Algorithm and Sim-to-Real Transfer for Active Autonomous Driving** — *HYUNDAI MOBIS Youth Engineering Academic Conference 2025*
- **Lane Image Frame Prediction and Noise Reduction for High-Speed AV Environments** — *HYUNDAI MOBIS Youth Engineering Academic Conference 2025*
- **Where We Are Is Real** — *HYUNDAI MOBIS Youth Engineering Academic Conference 2024*
- **Graph Theoretical Analysis of Torus with NetworkX** — *Korea Software Congress (KCC) 2023*

## Awards

- Grand Prize · Youth Engineering Academic Conference, HYUNDAI MOBIS (Jul. 2025)
- Excellence Award · Youth Engineering Academic Conference, HYUNDAI MOBIS (Jul. 2025)
- Bronze Award · KSEF Science Project Competition (Feb. 2025)
- Excellence Award · Youth Engineering Leader Autonomous Vehicle Competition, HYUNDAI MOBIS (Nov. 2024)
- Silver Award · National Student Design Competition, KSME (Oct. 2024)
- Encouragement Award · Korea Software Congress, KIISE (Jul. 2024)
- Excellence Award · Youth Engineering Academic Conference, HYUNDAI MOBIS (Jul. 2024)
- Bronze Award · National Student Design Competition, KSME (Oct. 2023)

-----

<p align="center">
  <img height="180" src="https://ghstats.dev/api/langs?username=hiseyong&theme=dracula&layout=compact&hide_border=true&max_langs=6" alt="Top languages">
  <img height="180" src="https://streak-stats.demolab.com/?user=hiseyong&theme=dark&hide_border=true&background=00000000&ring=58A6FF&fire=FF6B35&currStreakLabel=58A6FF" alt="GitHub streak">
  <br>
  <img height="180" src="https://ghstats.dev/api/card?username=hiseyong&theme=dracula&hide_border=true&show_icons=true" alt="GitHub stats">
  <br>
  <img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=hiseyong&bg_color=00000000&color=58a6ff&line=58a6ff&point=ffffff&area=true&hide_border=true&custom_title=Contribution%20Graph" alt="Contribution activity graph">
</p>

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=120&section=footer" />
